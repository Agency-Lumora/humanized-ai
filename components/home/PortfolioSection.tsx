  ...
  return (
    <section id="portfolio" className="relative overflow-hidden bg-[#F4EFE7] py-[clamp(4rem,8vw,6rem)]">
      <div className="mx-auto max-w-[80rem] px-6 sm:px-8 lg:px-12">
        <motion.header initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="mb-[clamp(2.5rem,5vw,4rem)] flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="grid gap-10 border-t border-[#806C5D]/25 lg:grid-cols-3 lg:gap-8">
          {projects.map((project, index) => (
            <motion.article key={project.number} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.8, delay: index * 0.1, ease: [0.22, 1, 0.36, 1] }} className="border-b border-[#806C5D]/25 pt-6 lg:border-b-0 lg:border-r lg:pr-8 lg:last:border-r-0">
